...
class PatchEmbed(layers.Layer):
    """
    PatchEmbed包含patch partition和Linear embedding两部分
    ...
    def __init__(self, patch_size=4, embed_dim=96, norm_layer=None):
        ...
        self.norm = norm_layer(epsilon=1e-6, name="norm") if norm_layer else layers.Activation('linear')#linear
        ...
        self.proj = layers.Conv2D(filters=embed_dim, kernel_size=patch_size,
                                  ...
                                  name="proj")
#正向传播过程,H和W是传入图像的高度和宽度，支持多尺度体现在这
    def call(self, x, **kwargs):
        _, H, W, _ = x.shape

        # padding
        # 如果输入图片的H，W不是patch_size的整数倍，需要进行padding填充
...
class PatchMerging(layers.Layer):
    ...
    """
    def __init__(self, dim: int, norm_layer=layers.BatchNormalization, name=None):#LayerNormalization
...
#forward，x是输入的数据，H高度，W宽度
    def call(self, x, H, W):
        ...
        x = self.norm(x)
        x = self.reduction(x)  # [B, H/2*W/2, 2*C]后可加dropout尝试性能
...
class MLP(layers.Layer):
    ...
    def __init__(self, in_features, mlp_ratio=4.0, drop=0., name=None):
        ...
        self.act = layers.Activation("gelu")#gelu
...
class SwinTransformerBlock(layers.Layer):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        # Layer Nom
        self.norm1 = layers.BatchNormalization(epsilon=1e-6, name="norm1")#LayerNormalization
        #WindowAttention：Multi-head attention block，Wmsa或swmsa
        ...
        self.drop_path = layers.Dropout(rate=drop_path, noise_shape=(None, 1, 1)) if drop_path > 0. \
            else layers.Activation("linear")#linear
        # 第二个Layer Nom
        self.norm2 = layers.BatchNormalization(epsilon=1e-6, name="norm2")#LayerNormalization
...
class BasicLayer(layers.Layer):
    ...
    def call(self, x, H, W, training=None):
        attn_mask = self.create_mask(H, W)  # [nW, Mh*Mw, Mh*Mw]，生成mask蒙版
...
class SwinTransformer(Model):
    ...
    """
    """patch_size是下采样的倍率，num_classes分类类别数，embed_dim对应linear embedding后的C，depths是每一个block中重复swinTransformerblock的次数
        num_heads每个swinTblock中multihead的头数，window_size是WMSA和swmsa的窗口大小，mlp_ratio是在全连接层中把Chanel翻多少倍
        qkv_bias在multihead中是否使用偏置
        drop_path_rate在每一个swinTblock中使用
    """
    def __init__(self, patch_size=4, num_classes=1000,
                 embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24),
                 window_size=7, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=layers.BatchNormalization, name=None, **kwargs):#LayerNormalization
        super().__init__(name=name)
        ...
        # stochastic depth decay rule，linspace官方方法，0初始数值，drop_path_rate末尾数值，depths步长，自动生成每个swint的droprate
        dpr = [x for x in np.linspace(0, drop_path_rate, sum(depths))]

        # build layers，遍历生成每个stage，i_layer从0到3，创建了所有的layer
        self.stage_layers = []
        #这个循环生成所有的stage
    ...
    def call(self, x, training=None):
        ...
        # 加上PCA的
        # x3 = Flatten()(feature_pca)
        # x = layers.concatenate([x,x3])

        # 加上卷积的  内存不够
        # x3 = layers.Conv2D(filters=16, kernel_size=(6, 6), activation='relu',strides= (6,6),
        #                   input_shape=(1, 224, 224, 3))(x)
        # x3 = layers.MaxPooling2D(6, 6)(x3)
        # x3 = Dropout(0.25)(x3)
        # x3 = Flatten()(x3)
        # x = layers.concatenate([x, x3])
        ...
        # # 注意力机制 # 内存不够
        # attention_module = 'cbam_block'  # 两种注意力机制模型 cbam_block / se_block
        # # attention_module = 'se_block'
        # attention = attention_module(x, attention_module)
