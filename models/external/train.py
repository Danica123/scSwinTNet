...
from dataread import train_feature_sequence_3
from dataread import train_label_int
...
def main():
    ...
    #输入图片大小img_size
    # img_size = 224
    batch_size = 64 #过小，会导致模型损失波动大，难以收敛，过大时，模型前期由于梯度的平均，导致收敛速度过慢，调大点
    epochs = 100
    num_classes = type_num
    freeze_layers = False
    # initial_lr = 0.0001 #学习率太小，需要提升学习率以加快收敛;过小，会训练缓慢
    initial_lr = 0.0001
    weight_decay = 1e-5


    # 每次运行后都做一记录在logs文件夹，按照运行时间建立文件夹
    log_dir = "./new_tiny_logs/mouse_Kidney7926_epoch100_224.ckpt/"
    ...
    train_ds, val_ds = generate_ds(train_feature_sequence_3,
                                   train_label_int,
                                   train_im_width=224,
                                   train_im_height=224,
                                   # train_im_width=160,
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy.result() > best_val_acc:
            ...
            #newHuman
            save_name = "./new_tiny_weights/mouse_Kidney7926_epoch100_224.ckpt"
            # c_callback = tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', patience=10, verbose=0, mode='max')
